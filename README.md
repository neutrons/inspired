...
## Installation:
INSPIRED currently only runs on Linux operating systems. The neutron data server ([analysis cluster](https://analysis.sns.gov)) at ORNL will work if you can access it. We are working on deploying INSPIRED on the analysis cluster so that all users can use it easily. Before the deployment is completed, you may install INSPIRED in your home directory on the analysis cluster (or any other Linux machine) by following the steps below ([Option 1](https://github.com/cyqjh/inspired/main/README.md#option-1)). Alternatively, you may download a pre-compiled VirtualBox image and run INSPIRED as a virtual machine (VM) on any platform, including Windows, MacOS, and Linux ([Option 2](https://github.com/cyqjh/inspired/main/README.md#option-2)).
