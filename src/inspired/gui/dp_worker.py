...
from inspired.gui.latent_space_model import Decoder, PeriodicNetwork
...
class DPWorker:
    ...
    def build_data(self, entry, lsv=np.zeros(50), r_max=5.0):
    ...
    def savenplot(
        ...
        if self.data["type"] == "Phonon DOS" or self.data["type"] == "VISION spectrum":
            ...
            if self.data["type"] == "Phonon DOS" and partial_dos and self.pdos:
                ...
                f = open(os.path.join(cwd, outfile), "w")
                print(
                    "# Energy (" + xlabel + "), Normalized spectrum (" + ",".join([s for s in self.species]) + ")",
                    file=f,
                )
                for i in range(len(self.data["x"])):
                    print(
                        str(xaxis[i]) + ",",
                        ",".join([str(self.pdos[s][i]) for s in self.species]),
                        file=f,
                    )
                f.close()
